<template>
  <div class="min-h-screen bg-bg flex">
    <AppSidebar :nav-items="navItems" role-label="السوبر أدمن" />
...
<script>
import { LayoutDashboard, Users, GraduationCap, Settings } from 'lucide-vue-next'
...
export default {
  ...
  data() {
    return {
      /**
       * عناصر التنقل — تُوسَّع كلما وُصفت صفحة جديدة.
       * الشكل: { label, to, icon, exact } أو مجموعات { title, items: [] }
       */
      navItems: [
        { label: 'لوحة المعلومات', to: '/super-admin', icon: LayoutDashboard, exact: true }
